// components/StatCard.js — Stat card with inline SVG icons and premium styling
const ICONS = {
    navy: (
...
export default function StatCard({ icon, value, label, sub, color = 'navy' }) {
    ...
    return (
        <div className={`card border-l-4 ${border} flex items-center gap-3 p-4 sm:p-6 shadow-sm hover:shadow-md transition-shadow`}>
            <div className="shrink-0">
                {svgIcon || <span className="text-2xl sm:text-3xl">{icon}</span>}
            </div>
